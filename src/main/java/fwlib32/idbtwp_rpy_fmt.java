package fwlib32;
import com.sun.jna.NativeLong;
import com.sun.jna.Pointer;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class idbtwp_rpy_fmt extends Structure {
	public double[] orign = new double[3];
	public double i;
	public double j;
	public double k;
	public short turn;
	public short reserve0;
	public NativeLong[] reserve = new NativeLong[23];
	public idbtwp_rpy_fmt() {
		super();
	}
	protected List<String> getFieldOrder() {
		return Arrays.asList("orign", "i", "j", "k", "turn", "reserve0", "reserve");
	}
	public idbtwp_rpy_fmt(double orign[], double i, double j, double k, short turn, short reserve0, NativeLong reserve[]) {
		super();
		if ((orign.length != this.orign.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.orign = orign;
		this.i = i;
		this.j = j;
		this.k = k;
		this.turn = turn;
		this.reserve0 = reserve0;
		if ((reserve.length != this.reserve.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.reserve = reserve;
	}
	public idbtwp_rpy_fmt(Pointer peer) {
		super(peer);
	}
	public static class ByReference extends idbtwp_rpy_fmt implements Structure.ByReference {
		
	};
	public static class ByValue extends idbtwp_rpy_fmt implements Structure.ByValue {
		
	};
}
