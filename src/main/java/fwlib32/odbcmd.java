package fwlib32;
import com.sun.jna.NativeLong;
import com.sun.jna.Pointer;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class odbcmd extends Structure {
	public byte adrs;
	public byte num;
	public short flag;
	public NativeLong cmd_val;
	public NativeLong dec_val;
	public odbcmd() {
		super();
	}
	protected List<String> getFieldOrder() {
		return Arrays.asList("adrs", "num", "flag", "cmd_val", "dec_val");
	}
	public odbcmd(byte adrs, byte num, short flag, NativeLong cmd_val, NativeLong dec_val) {
		super();
		this.adrs = adrs;
		this.num = num;
		this.flag = flag;
		this.cmd_val = cmd_val;
		this.dec_val = dec_val;
	}
	public odbcmd(Pointer peer) {
		super(peer);
	}
	public static class ByReference extends odbcmd implements Structure.ByReference {
		
	};
	public static class ByValue extends odbcmd implements Structure.ByValue {
		
	};
}
